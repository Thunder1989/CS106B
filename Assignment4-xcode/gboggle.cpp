...
const string START_BUTTON_COLOR = "#00FF50";

const double START_BUTTON_X = 200.0;    /* upper left corner of start button  */
const double START_BUTTON_Y = 275.0;    /* upper right corner of start button */
const double START_BUTTON_WIDTH = 150.0;/* width of start button              */
const double START_BUTTON_HEIGHT = 40.0;/* height of start button             */
 ...
 * ----
 * This function displays the winner of the game and the number of points the player won by.
 */
void calculateWinner() {
    int difference = (int) abs(gState.scores[COMPUTER] - gState.scores[HUMAN]);
    if (gState.scores[COMPUTER] > gState.scores[HUMAN])
        cout << "I win by " << difference << "!" << endl;
    else if (gState.scores[HUMAN] > gState.scores[COMPUTER])
        cout << "You win by " << difference << "..." << endl;
    else
        cout << "Wait, we tied?!?!" << endl;
}

/* Function: drawCenteredString
 * ---------
 * This function displays a string centered on the given coordinates.
 */
void drawCenteredString(double xc, double yc, string str, string color) {
    int mid = str.size()/2;
    for (int i = -mid; i <= mid; i++) {
        drawCenteredChar(xc + i * 25.0, yc, str[mid+i], color);
    }
}

/* Function: drawStartButton
 * -----
 * This function draws a start button below the Boggle board.  The color depends on bool flag.
 */
void drawStartButton(bool flag) {
    drawAndFillRoundedRect(START_BUTTON_X, START_BUTTON_Y, START_BUTTON_WIDTH, START_BUTTON_HEIGHT, 5.0, flag ? BOARD_COLOR : START_BUTTON_COLOR);
    drawCenteredString(START_BUTTON_X + START_BUTTON_WIDTH/2, START_BUTTON_Y + START_BUTTON_HEIGHT/2, "START", LETTER_COLOR);
}
 ...
 * This program plays a game of Boggle or Big Boggle.  It prompts the user for parameters to set up the board.  The human player gets the first turn and selects words.  If the words are valid (English lexicon, have not been used before, at least 4 letters, and can be constructed on the Boggle board), the human player is awarded points.  Once the human player is finished, the computer takes its turn at finding all other possible words subject to the same constraints.  Finally, the program displays the winner and gives the user the option of playing again.
 */
...
const double START_BUTTON_LEFT = 200.0;         // left boundary of Start button
const double START_BUTTON_RIGHT = 350.0;        // right boundary of Start button
const double START_BUTTON_TOP = 275.0;          // top boundary of Start button
const double START_BUTTON_BOTTOM = 315.0;       // bottom boundary of Start button
 ...
 */
void turnHuman(const Lexicon & lex, Grid<char> & boggleChars, int dim, Lexicon & prevWords) {
    cout << endl << "I'll let you go first!" << endl;
...
/* Function: waitForClickStart
 * Usage: waitForClickStart()
 * -----
 * This function draws a Start button and dismisses the process until the user clicks the button.
 */
void waitForClickStart() {
    drawStartButton(false);
    cout << "Click the START button to begin." << endl;
    while (true) {
        GMouseEvent e = waitForEvent(CLICK_EVENT);
        double xc = e.getX();
        double yc = e.getY();
        if (START_BUTTON_LEFT <= xc && xc <= START_BUTTON_RIGHT && START_BUTTON_TOP <= yc && yc <= START_BUTTON_BOTTOM) break;
    }
